...
const HowItWorks: React.FC = () => {
  ...
  return (
    <div className="bg-[var(--color-bg-surface)] rounded-2xl p-8">
      <h3 className="text-center text-lg font-bold text-[var(--color-text-dark)] mb-8">
        {t('home.howItWorksTitle')}
      </h3>
      <div className="relative grid grid-cols-4 gap-2">
        <div className="absolute top-[19px] left-[15%] right-[15%] h-px bg-[var(--color-border)] z-0" />
        {steps.map((step, i) => (
          <div key={step.number} className="relative z-10 flex flex-col items-center text-center px-1 animate-fade-in-up" style={{ animationDelay: `${i * 0.12}s` }}>
            ...
            <h4 className="mt-3 text-sm font-semibold text-[var(--color-text-dark)]">{step.title}</h4>
            <p className="mt-1 text-xs leading-5 text-[var(--color-text-muted)]">{step.description}</p>
          </div>
        ))}
      </div>
