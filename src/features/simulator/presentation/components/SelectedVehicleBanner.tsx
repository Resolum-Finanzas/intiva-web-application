    ...
    <div className="bg-primary rounded-xl p-5 relative overflow-hidden">
      <div className="flex justify-between items-center relative z-10">
        ...
        <div className="text-right">
          <p className="text-xs text-white/70 uppercase tracking-wider">{t('simulator.vehiclePrice')}</p>
          <p className="text-3xl font-bold text-white">${fmt(vehiclePrice)}</p>
        </div>
      </div>
      <Car size={120} className="text-white/10 absolute -bottom-6 -right-6 animate-pulse-soft" />
    </div>
  );
};
